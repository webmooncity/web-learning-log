...
* 리액트에서 **훅**(hook)이란? 함수 컴포넌트에서 리액트의 기능(상태, 생명주기, 컨텍스트 등)을 걸어 쓰게 해주는 함수들

---

## 11.6, 상태 업데이트 최적화

* useDeferredValue 훅, useTransition 훅

### 11.6.1, useDeferredValue 훅

* useDeferredValue 훅
  - 형식: `const deferredValue = useDeferredValue(value, 초깃값);`
  - useDeferredValue 훅: 입력 값의 실시간 반영을 지연시켜 무거운 컴포넌트의 리렌더링을 최적화함.

### 11.6.2, useTransition 훅

* useTransition 훅: 리액트에서 UI의 일부 렌더링 작업을 백그라운드에서 처리할 수 있도록 함.
  - 형식 `const [isPending, startTransition] = useTransition();`

* useDeferredValue 훅은 특정 값 하나만 지연해 렌더링을 최적화할 수 있는 반면, useTransition 훅은 함수 단위의 작업 전체를 지연할 수 있고, 작업의 진행 상태를 확일할 수 있는 isPending 값도 함께 제공함.
  * 지연하려는 대상이 '값'인지 '작업 전체'인지에 따라 적절한 훅을 선택해야 함.
