...
- 아이디 선택자 : #
* id 속성값은 고유해야 함
- 클래스 선택자 : 속성값 앞에 . 기호를 붙임
  ```
  .class속성값{/* CSS 코드 */}
  ```
  - id 속성과 다르게 속성값을 중복해서 사용할 수 있음
...
- 기본 속성 선택자 : HTML 태그에서 사용할 수 있는 속성과 값을 사용해 선택자를 지정하는 방법
  ...
  태그[속성=값]{/* CSS 코드 */}
  ```
* 클래스 선택자와 아이디 선택자도 각각 class 속성과 id 속성으로 선택자를 지정하므로, 넓은 범위에서는 속성 선택자에 포함됨.
* 속성 선택자는 아이디, 태그, 클래스 선택자와 함께 사용 가능. 단독 사용도 가능.
- 문자열 속성 선택자
